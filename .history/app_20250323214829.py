...
# 📄 **Resume Generator**
    ...
    # ✅ FIXED: Ensure only valid dictionaries with 'text' are processed
    saved_achievements_text = "\n".join(
        [ach["text"] for ach in saved_achievements if isinstance(ach, dict) and "text" in ach]
    ) if saved_achievements else ""

    # Merge both user-input and saved achievements
    all_achievements = saved_achievements_text + ("\n" + user_achievements if user_achievements else "")
    ...
    if st.button("📜 Generate Resume"):
        resume_pdf = generate_resume(
            ...
            projects=projects,
            achievements=all_achievements  # 🔥 Including Merged Achievements Here
        )
