...
# Sidebar Navigation (Added Portfolio)
...
if page == "🏠 Home":
    ...
    if uploaded_file:
        if uploaded_file.type.startswith("image/"):
            ...
            st.image(image, caption="🖼 Uploaded Certificate", use_container_width=True)  # Updated parameter
            ...
            for idx, image in enumerate(images):
                st.image(image, caption=f"📄 Page {idx + 1}", use_container_width=True)  # Updated parameter
                extracted_text = extract_text_from_image(image)
                st.write(f"📝 Extracted Text from Page {idx + 1}:", extracted_text)

                if st.button(f"➕ Save Page {idx + 1} Certificate"):
    ...
    dob = st.date_input("📅 Date of Birth")  # Added D.O.B field
    ...
    if st.button("📜 Generate Resume"):
        ...
        resume_pdf = generate_resume(name, dob, email, phone, address, skills, education, experience, achievements)
