...
if "portfolio" not in st.session_state:
    st.session_state.portfolio = []  # Store uploaded certificates
...
if page == "🏠 Home":
    ...
    if uploaded_file:
        cert_info = {"name": uploaded_file.name, "type": uploaded_file.type}
        ...
        if uploaded_file.type.startswith("image/"):
            ...
            cert_info["image"] = image
            cert_info["text"] = extracted_text

        elif uploaded_file.type == "application/pdf":
            images = extract_images_from_pdf(uploaded_file)
            cert_info["images"] = images
            extracted_texts = []
            for idx, image in enumerate(images):
                st.image(image, caption=f"📄 Page {idx + 1}", use_column_width=True)
                text = extract_text_from_image(image)
                extracted_texts.append(text)
                st.write(f"📝 Extracted Text from Page {idx + 1}:", text)
            cert_info["text"] = extracted_texts

        if st.button("➕ Add to Portfolio"):
            st.session_state.portfolio.append(cert_info)  # Store in session state
            st.success("✅ Certificate added to portfolio!")

elif page == "📁 Portfolio":
    st.header("📁 Your Portfolio")
    
    if not st.session_state.portfolio:
        st.warning("⚠️ No certificates added yet.")
    else:
        for cert in st.session_state.portfolio:
            st.subheader(f"📜 {cert['name']}")
            
            if cert["type"].startswith("image/"):
                st.image(cert["image"], use_column_width=True)
            elif cert["type"] == "application/pdf":
                for idx, image in enumerate(cert["images"]):
                    st.image(image, caption=f"📄 Page {idx + 1}", use_column_width=True)

            st.write("📝 Extracted Text:", cert["text"])
